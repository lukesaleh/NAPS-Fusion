...
class MassFunction(dict):
    ...
    def from_q(q):
        ...
        for h1 in q.keys():
            v = fsum(
                [q[h1 | h2] * (-1) ** (len(h2 - h1)) for h2 in powerset(frame - h1)]
            )
            if v > 0:
                m[h1] = v
        mass_sum = fsum(m.values())
    ...
    def core(self, *mass_functions):
        ...
        if mass_functions:
            return frozenset.intersection(
                self.core(), *[m.core() for m in mass_functions]
            )
        else:
            focal = self.focal()
            if not focal:
    ...
    def bel(self, hypothesis=None):
        ...
        if hypothesis is None:
            ...
            if not hypothesis:
                ...
                return fsum(
                    [v for (h, v) in self.items() if h and hypothesis.issuperset(h)]
                )
    ...
    def _combine(
        self, mass_function, rule, normalization, sample_count, importance_sampling
    ):
    ...
    def combine_gbt(
        ...
        if sample_count == None:  # deterministic
            return self.combine_conjunctive(
                MassFunction.gbt(likelihoods), normalization=normalization
            )
        else:  # Monte-Carlo
            if (
                not normalization
    ...
    def conflict(self, mass_function, sample_count=None):
        ...
        m = self.combine_conjunctive(
            mass_function, normalization=False, sample_count=sample_count
        )
        empty = m[frozenset()]
        m_sum = fsum(m.values())
        diff = m_sum - empty
    ...
    def pignistic_inverse(p):
        ...
        poss = MassFunction(
            {h1: fsum([min(p[h1], p[h2]) for h2 in p.keys()]) for h1 in p.keys()}
        )
    ...
    def from_samples(histogram, method="idm", alpha=0.05, s=1.0):
        ...
        if not histogram:
            return MassFunction()
        if (
            sum(histogram.values()) == 0
        ):  # return vacuous/uniform belief if there are no samples
    ...
    def _from_samples_maxbel(histogram, alpha, ordered=False):
        ...
        if ordered:
            ...
            # constraint (24)
            bel = lambda index, m: fsum(
                m[sum([2**i for i in h_ind])] for h_ind in powerset(singletons(index))
            )
            c24 = lambda m, i: p_lower_set(MassFunction._from_array_index(i, H)) - bel(
                i, m
            )
            for i in range(L):
            ...
            for k, h_k in enumerate(H):
                ...
                for A in powerset(P_k):
                    ...
                    for i, h in enumerate(H):
                        cons.append(
                            partial(
                                lambda p, i_s, p_s: p[i_s] - p_s, i_s=i, p_s=p_lower[h]
                            )
                        )  # lower bound
                        cons.append(
                            partial(
                                lambda p, i_s, p_s: p_s - p[i_s], i_s=i, p_s=p_upper[h]
                            )
                        )  # upper bound
                    # constraint (27)
...
def gbt_pl(hypothesis, likelihoods, normalization=True):
    ...
    eta = _gbt_normalization(likelihoods) if normalization else 1.0
    return eta * (
        1.0 - reduce(mul, [1.0 - l[1] for l in likelihoods if l[0] in hypothesis], 1.0)
    )
...
def gbt_pignistic(singleton, likelihoods):
    ...
    if singleton_lh is None:
        raise ValueError(
            "singleton %s is not contained in likelihoods" % repr(singleton)
        )
